...
async def test_notifications_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if notification_scheduler is None:
            ...
            message_text += "❌ Планировщик уведомлений недоступен.\n\n"
            message_text += "🔧 <b>Возможные причины:</b>\n"
            message_text += "• Ошибка инициализации планировщика\n"
            message_text += "• Проблемы с Google Sheets\n"
            message_text += "• Неправильная конфигурация"
        else:
            # Устанавливаем chat_id для планировщика
