...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.callback_query:
        ...
        await update.callback_query.edit_message_text("🔄 Загружаю актуальную статистику...")
    
    # Получаем еженедельную сводку (всегда свежую)
    ...
    if weekly_summary:
        ...
        if weekly_summary['lessons_this_week']:
            ...
            # Сортируем по дате и показываем первые 5
            from datetime import datetime
            lessons_sorted = sorted(weekly_summary['lessons_this_week'], 
                                  key=lambda x: datetime.strptime(x['date'], '%d.%m.%Y'))
            
            for lesson in lessons_sorted[:5]:
                status_emoji = "✅" if lesson['status'] in ['✔️', 'Присутствовал'] else \
                              "❌" if lesson['status'] in ['✖️', 'Отсутствовал'] else "📅"
        ...
        if weekly_summary['payments_this_week']:
            ...
            message_text += f"• Общая сумма: {weekly_summary['total_budget']} руб.\n"
        ...
        # Если нет еженедельной сводки, пытаемся её получить
        try:
            weekly_summary = sheets_service.get_weekly_summary()
            if weekly_summary:
                # Формируем сообщение с еженедельной сводкой (копируем логику сверху)
                stats = weekly_summary['attendance_stats']
                message_text = f"📊 <b>СВОДКА НА НЕДЕЛЮ</b>\n"
                message_text += f"📅 {weekly_summary['week_start']} - {weekly_summary['week_end']}\n\n"
...
async def fix_duplicate_ids_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("📅 Google Календарь", callback_data="menu_google_calendar")],
    ...
    try:
        ...
        if not active_subs:
            ...
            for i, sub in enumerate(active_subs, 1):
                ...
                try:
                    total_lessons = int(sub.get('Количество занятий', 0) or 0)
                except (ValueError, TypeError):
                    total_lessons = 0
                
                try:
                    remaining_lessons = int(sub.get('Осталось занятий', 0) or 0)
                except (ValueError, TypeError):
                    remaining_lessons = 0
                    
                try:
                    cost = float(sub.get('Стоимость', 0) or 0)
                except (ValueError, TypeError):
                    cost = 0
                
                # Статистика посещений
                try:
                    attended = int(sub.get('Посещено', 0) or 0)
                except (ValueError, TypeError):
                    attended = 0
                    
                try:
                    missed = int(sub.get('Пропущено', 0) or 0)
                except (ValueError, TypeError):
                    missed = 0
...
async def confirm_delete_subscription_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == 'confirm_delete_yes':
        try:
            ...
            # Показываем alert с кратким результатом
...
async def create_sub_start_minute_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Создаем клавиатуру с часами окончания (от start_hour+1 до 22:00)
    time_keyboard = []
    for hour in range(start_hour + 1, 23):
        time_keyboard.append([InlineKeyboardButton(f"{hour:02d}:xx", callback_data=f"end_hour_{hour}")])
...
async def create_sub_end_hour_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    message_text = (f"👤 Ребенок: <b>{context.user_data['new_sub']['child_name']}</b>\n"
                    ...
                    f"Настройка расписания для <b>{day_names[day_num]}</b>\n"
                    f"⏰ Время начала: <b>{start_hour:02d}:{start_minute:02d}</b>\n"
                    f"⏰ Час окончания: <b>{end_hour:02d}:xx</b>\n\n"
                    "Выберите минуты окончания занятия:")
    
    # Создаем клавиатуру с минутами (каждые 5 минут)
    minute_keyboard = []
    for minute in [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]:
        minute_keyboard.append([InlineKeyboardButton(f"{end_hour:02d}:{minute:02d}", callback_data=f"end_minute_{minute}")])
